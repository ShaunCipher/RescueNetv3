...
class MainWorkspace(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.routing_engine = RoutingManager(self.fig, self.ax, self.nodes, self.data_engine.edges_df)

        self.report_manager = AccidentManager(
            fig=self.fig,
            ax=self.ax,
            master_nodes=self.nodes,
        ...
        self.filter_engine = FilterLogic(categories=get_category_order(), plots=self.plots, canvas=self.canvas)
    ...
    def setup_map(self):
        ...
        try:
            map_path = os.path.join(self.base_dir, "img", "map.png")
    ...
    def refresh_accident_plot(self):
        ...
        acc_path = os.path.join(self.base_dir, "data", "accidents.csv")
        if os.path.exists(acc_path):
            try:
                ...
                self.log_analysis(f"Error drawing accidents: {exc}")
